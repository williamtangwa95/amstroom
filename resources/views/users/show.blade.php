        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table table-borderless" style="font-size:.85rem;">
                    ...
                    <tr><th style="color:var(--text-secondary);">Role</th><td><span class="role-pill role-{{ $user->role }}">{{ str_replace('_',' ',ucfirst($user->role)) }}</span></td></tr>
                    <tr><th style="color:var(--text-secondary);">Assigned Shop</th><td>{{ $user->shop ? $user->shop->shop_name : 'Unassigned' }}</td></tr>
                    <tr><th style="color:var(--text-secondary);">Total Sales Made</th><td><strong style="color:#3fb950;">{{ $user->sales->count() }} transaction(s)</strong></td></tr>
                    <tr><th style="color:var(--text-secondary);">Defects Reported</th><td>{{ $user->defects->count() }} item(s)</td></tr>
                </table>

...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <span><i class="bi bi-exclamation-octagon me-2" style="color:#e94560;"></i>Defective Items Audit Log</span>
...
<!-- Modal: Send Filtered Defect Report to Email -->
        ...
        <div class="modal-content shadow-lg border-0">
            <div class="modal-header bg-dark text-white border-bottom border-secondary">
                <h5 class="modal-title fs-6 fw-bold" id="emailReportModalLabel">
                    <i class="bi bi-envelope-paper me-2 text-info"></i> Send Defect Report
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="sendDefectReportEmailForm">
                @csrf
                <div class="modal-body py-3">
                    <div class="p-3 bg-light rounded-3 mb-3 border">
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-2">
                            <span class="badge bg-secondary-subtle text-dark border"><i class="bi bi-shield-exclamation me-1"></i> Total Defective Units: {{ number_format($totalDefective) }}</span>
                            <span class="badge bg-secondary-subtle text-dark border"><i class="bi bi-list-check me-1"></i> Incidents: {{ $defects->count() }}</span>
                        </div>
                    </div>
                ...
                <div class="modal-footer bg-light border-top py-2">
                    ...
                    <button type="submit" class="btn btn-sm btn-accent d-flex align-items-center gap-1" id="btnSubmitSendDefectReportEmail">
...
$(() => {
    ...
    /* ── Send Defect Report AJAX ── */
    $('#sendDefectReportEmailForm').on('submit', function(e) {
        e.preventDefault();
        const $btn = $('#btnSubmitSendDefectReportEmail');
        ...
        $.ajax({
            url: "{{ route('reports.defect.send-email') }}",
            type: "POST",
            data: formData,
            success: function(res) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Report Sent!',
                    text: res.message || 'Defect report has been successfully sent to email.',
                    confirmButtonColor: '#0088cc'
                });
            },
            error: function(xhr) {
                $btn.prop('disabled', false).html(originalBtnHtml);
                let msg = 'Failed to send defect report email.';
